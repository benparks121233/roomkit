...
**Fix:** Add `signup_completed` and `quiz_completed` to `_ALLOWED_CLIENT_EVENTS`. Fire `trackEvent` after email confirmation callback and after quiz submit (before design generation).
**Owner:** CODE
**Ref:** `app/api/routes.py:1273-1275` (allowed events), `web/app/auth/callback/route.ts`, `web/app/design/page.tsx` (quiz submit handler)
